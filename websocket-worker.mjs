// Live chat websocket worker, based on Cloudflare's Edge Chat Demo Worker
	...
	async fetch(request, env) {
		return await handleErrors(request, async () => {
			...
			if (!path[0]) {
				...
				return new Response("This is the chat web socket server", {
		...
		case "room": {
			...
			if (!path[1]) {
				...
				if (request.method == "POST") {
					...
					return new Response("Specify room name", { status: 405 });
				}
			}
